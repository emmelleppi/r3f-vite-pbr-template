...
void main() {
	...
	vec3 diffuseColor = ((1.0 - 0.6 * invOpacity) * smoothedNdL +  0.5 * (1.0 - smoothedNdL)) * (u_color + 0.3 * invOpacity + NdL * 0.1) * invOpacity;
	...
	gl_FragColor.rgb += invOpacity * 0.25 * circle(gl_FragCoord.xy/1024.0, 0.0002);
